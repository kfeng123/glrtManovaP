...
Let $W=\tilde{J}^T Z^T Z \tilde{J}$ be the dual covariance matrix. 
Let $w_{ij}$ be the $(i,j)$th element of $W$.
We use
...
\end{lemma}
\begin{proof}
    For a matrix $A$, we denote by  $A_{[a:b,c:d]}$ the $a$-to-$b$-th row, $c$-to-$d$-th column of matrix $A$, by $A_{[a:b,:]}$ and $A_{[:,c:d]}$ the $a$-to-$b$-th full rows and $c$-to-$d$-th full columns of $A$ respectively.
Note that 
$
...
Note that
$\lambda_{\max}(G_{1[(r+1):p,:]}^T \Lambda_2 G_{1[(r+1):p,:]})\leq c_1\lambda_{\max}(G_{1[(r+1):p,:]}^T G_{1[(r+1):p,:]})=O_P(p)$ by Bai-Yin's law.
