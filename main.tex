...
For example, under the approximate factor model in \cite{Fan2013Large}, the test in \cite{Aoshima2018} requires $p/n \to 0$, while the test in \cite{WANG2018225} requires that $p/n^2\to 0$ and the small eigenvalues of $\bSigma$ are all equal.
   ...
   In high dimensional setting, both sum-of-squares type statistics and extreme value type statistics are not based on likelihood function.
    ...
    Compared with the work of \cite{Zhao2016A}, our main contribution is that we give a thoroughly theoretical analysis of the LFD test.
    ...
    It is shown that the LFD test has similar behavior to existing sum-of-squares tests under the nonspiked covariance, while significantly outperforms competing tests under the spiked covariance.
...
Then Lemma~\ref{optProp} in Appendix implies that for $p>n$,
...
\begin{theorem}
    \label{fenTheorem1}
    Suppose as $n,p\to \infty$, the condition \eqref{nonSpikedC} holds.
...
\end{equation*}
That is, the asymptotic variance of $T(\bX)$ is much smaller than the tests in \cite{Bai1996Efiect} and \cite{Chen2010A}.
...
We can see that the empirical power of the LFD test holds nearly constant as $\rho$ varies while the empirical powers of competing sum-of-squares type tests decrease as $\rho$ increases.
...
\begin{figure}[htbp]
    ...
    \caption{Empirical powers of tests. $\alpha=0.05$, $n_1=n_2=n_3=35$, $p=1000$.}
    \label{figure1}
\end{figure}
