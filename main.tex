...
\begin{proof}
    ...
    Note that $\bU_1$ is in fact the $r$ leading eigenvectors of $\hat{\bSigma}_0=\bU_1\bU_1^T \hat{\bSigma}\bU_1\bU_1^T + \bU_2\bU_2^T \hat{\bSigma}\bU_2\bU_2^T$ provided the eigenvalues of $\bU_1\bU_1^T \hat{\bSigma}\bU_1\bU_1^T$ are larger than the eigenvalues of $\bU_2\bU_2^T \hat{\bSigma}\bU_2\bU_2^T$. {\color{red}this is in fact true.}
    ...
    \begin{equation*}
        \|\hat{\bSigma}-\hat{\bSigma}_0 \|
        = n^{-1} \|\bLambda_1^{1/2} \bZ_A \bZ_B^T \bLambda_2^{1/2}\|
        = n^{-1} \|\bLambda_1^{1/2} (\bZ_A \bZ_A^T)^{1/2} \bL \bLambda_2^{1/2}\|,
    \end{equation*}
    where $\bL=(\bZ_A \bZ_A^T)^{-1/2}\bZ_A\bZ_B^T$ is a $r\times (p-r)$ random matrix with independent $\mathcal{N} (0,1)$ entries.
        Then we have
\begin{equation*}
    \begin{split}
        &\|\hat{\bSigma}-\hat{\bSigma}_0 \|
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
Thus, {\color{red} under certain conditions,} we have
\begin{equation*}
        \|\hat{\bSigma}-\hat{\bSigma}_0 \|
        =(1+o_P(1))\sqrt{\frac{\blambda_1 \mytr(\bLambda_2)}{n}}.
\end{equation*}
Then if $\blambda_1\mytr(\bLambda_2)/(n(\blambda_r-\blambda_{r+1})^2)< 10^{-2}$, the condition $\epsilon\leq 1/10$ will be satisfied with probability tending to $1$.
