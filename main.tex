...
$\max_{a^T a=1}\text{LR}_a=(1+\lambda_{\max}(FG^{-1}))^{n/2}$, which is an increasing function of Roy's maximum root test statistic.
...
\begin{aligned}
    ...
    &=\argmax_{a^T a=1,\text{LR}_a=+\infty} \Big(1+\frac{a^T F a}{a^T G a}\Big)^{n/2}\\
