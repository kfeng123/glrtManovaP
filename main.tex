...
Within the $i$th sample ($1\leq i\leq k$), the observations $\{X_{ij}\}_{j=1}^{n_i}$ are independent and identically distributed (i.i.d.) as $\mathcal{N}_p(\xi_i,\bSigma)$, the $p$-dimensional normal distribution with mean vector $\xi_i$ and common variance matrix $\bSigma$.
...
In another work,~\cite{Cai2014High} proposed a test statistic 
  ...
  %Suppose $\{X_{i1},\ldots, X_{in_i}\}$ are i.i.d.\ distributed as $\mathcal{N}(\xi_i,\bSigma)$ for $1\leq i\leq K$.
...
Let $\bW_{k-1}$ be a $(k-1)\times(k-1)$ symmetric random matrix whose entries above the main diagonal are i.i.d.\ $\mathcal{N}(0,1)$ and the entries on the diagonal are i.i.d.\ $\mathcal{N}(0,2)$.
...
%\begin{lemma}[\citet{DAVIDSON2001317} Theorem II.7]\label{DSbound}
    %Let $\bA$ be $m\times n$ random matrix with i.i.d. $\mathcal{N}(0,1)$ entries.
...
\begin{lemma}\label{lemma:con}
    Let $\{Z_i\}_{i=1}^n$ be i.i.d. $p$-dimensional random vectors with common distribution $\mathcal{N}_p(\mathbf{0}_p,\bI_p)$.
...
\begin{proposition}
Let $\bZ$ be a $p\times n$ random matrix with i.i.d. standard normal entries.
Then uniformly for $i=1,\ldots, r$, 
    \begin{equation*}
        %\lambda_i\left(n^{-1}\bZ^T \bLambda \bZ\right)
    \lambda_i(n^{-1}\bU\bLambda^{1/2} \bZ\bZ^T \bLambda^{1/2} \bU^T)
        =
        \blambda_i
        +
        n^{-1}\sum_{i=r+1}^p\blambda_i
        +O_P\left(\blambda_i \sqrt{\frac{r}{n}}+\frac{1}{\sqrt n}\|\bLambda_2\|_F+\|\bLambda_2\|_{\infty}\right).
    \end{equation*}
And we have
\begin{equation*}
    \begin{split}
     &\sum_{i=r+1}^p\lambda_i(n^{-1}\bU\bLambda^{1/2} \bZ\bZ^T \bLambda^{1/2} \bU^T)
    =
\mytr(\bLambda_2)
    -
    \frac{r}{n}\sum_{i=r+1}^p \blambda_i
    +O_P\left(\frac{r}{\sqrt{n}}\|\bLambda_2\|_F+r\|\bLambda_2\|_{\infty}\right)
    .
    \end{split}
\end{equation*}
\end{proposition}
\begin{proof}
    The positive eigenvalues of $n^{-1}\bU\bLambda^{1/2} \bZ\bZ^T \bLambda^{1/2} \bU^T$ are equal to the positive eigenvalues of $n^{-1}\bZ^T \bLambda \bZ$.
    Let $\bZ =(\bZ_A^T,\bZ_B^T)^T$, where $\bZ_A$ and $\bZ_B$ are the first $r$ rows and last $p-r$ rows of $\bZ$.
    We have
    \begin{equation*}
        \begin{split}
        &n^{-1}\bZ^T \bLambda \bZ=
        n^{-1}\bZ_A^T \bLambda_1 \bZ_A+
        n^{-1}\bZ_B^T \bLambda_2 \bZ_B
        \\
        =&
        n^{-1}\bZ_A^T \bLambda_1 \bZ_A+
        n^{-1}(\sum_{i=r+1}^p\blambda_i)\bI_n+
        n^{-1}\left(\bZ_B^T \bLambda_2 \bZ_B-(\sum_{i=r+1}^p\blambda_i)\bI_n\right)
        .
        \end{split}
    \end{equation*}
    It follows from Weyl's inequality that for $ i=1,\ldots, r$,
    \begin{equation}\label{eigenBoundForF}
        ...
        \lambda_i\left(n^{-1}\bZ^T \bLambda \bZ\right)
        -
        \lambda_i(n^{-1}\bZ_A^T \bLambda_1 \bZ_A)
        -
        n^{-1}\sum_{i=r+1}^p\blambda_i
        \right|
        \leq
        ...
        \end{split}
    \end{equation}
    Now we deal with $\lambda_i(n^{-1}\bZ_A^T \bLambda_1 \bZ_A)$, $ i=1,\ldots, r$.
    Using Weyl's inequality, we can derive the following lower bound.
\begin{equation*}\label{eq:DLower}
\begin{aligned}
&\lambda_i(\bZ_{A}^T \bLambda_1 \bZ_{A})
\geq
\lambda_i(\bZ_{A}^T \mydiag(\blambda_i \bI_{i},\bO_{(r-i)\times(r-i)}) \bZ_{A})
\\
    =&
    \lambda_i\Big( \blambda_i \bZ_{A}^T \bZ_{A}-\blambda_i\bZ_{A}^T \mydiag(\bO_{i\times i}, \bI_{r-i}) \bZ_{A}\Big)\\
    \geq&
    \lambda_r\Big( \blambda_i \bZ_{A}^T \bZ_{A}\Big)+\lambda_{p+i-r}\Big(-\blambda_i\bZ_{A}^T \mydiag(\bO_{i\times i}, \bI_{r-i}) \bZ_{A}\Big)\\
= &
\blambda_i \lambda_r(\bZ_{A}\bZ_{A}^T).
\end{aligned}
\end{equation*}
...
%\lambda_1(\bZ_{1[1:r,:]}^T \mydiag(\bO_{(i-1)\times(i-1)},\lambda_i,\ldots,\lambda_r) \bZ_{1[1:r,:]})
    \leq&
\lambda_1(\bZ_{A}^T \mydiag(\bO_{(i-1)\times(i-1)},\blambda_i \bI_{r-i+1}) \bZ_{A})
\leq  \blambda_i \lambda_1(\bZ_{A}\bZ_{A}^T).
\end{aligned}
\end{equation*}
...
\begin{equation}\label{eigenBoundForA}
    \begin{aligned}
\left|
\lambda_i(n^{-1}\bZ_{A}^T \bLambda_1 \bZ_{A})-\blambda_i
\right|
\leq&
\blambda_i 
\max\left(
    |\lambda_1(n^{-1}\bZ_A \bZ_A^T)-1|,
    |\lambda_r(n^{-1}\bZ_A \bZ_A^T)-1|
\right)
\\
=&\blambda_i \|n^{-1}\bZ_A\bZ_A^T -\bI_r\|.
    \end{aligned}
\end{equation}
Combining the equalities~\eqref{eigenBoundForF} and~\eqref{eigenBoundForA}, we have for $i=1,\ldots,r$,
\begin{equation*}
    \begin{split}
        &
        \left|
...
\begin{align}
            \notag
            \|n^{-1}\bZ_A\bZ_A^T -\bI_r\|&=
            O_P\left(\sqrt{\frac{r}{n}}+\frac{r}{n}\right)=O_P\left(\sqrt{\frac{r}{n}}\right),
        \\
        \label{conB2B}
        n^{-1}\left\|\bZ_B^T \bLambda_2 \bZ_B-(\sum_{i=r+1}^p\blambda_i)\bI_n\right\|&=O_P\left(\frac{1}{\sqrt{n}}\|\bLambda_2\|_F+\|\bLambda_2\|_{\infty}\right).
\end{align}
    This complets the proof of the first statement.
...
\begin{equation*}
    ...
    \\
    =&
    \mytr (n^{-1}\bZ_A^T \bLambda_1 \bZ_A)
    +
    \mytr (n^{-1}\bZ_B^T \bLambda_2 \bZ_B)
    -\sum_{i=1}^r\lambda_i(n^{-1}\ \bZ^T \bLambda \bZ)
    .
    \end{split}
\end{equation*}
The inequalities~\eqref{eigenBoundForF} and~\eqref{conB2B} implies that the last term satisfies
\begin{equation*}
    \begin{split}
    &\left|
    \sum_{i=1}^r\lambda_i(n^{-1}\ \bZ^T \bLambda \bZ)
    -\mytr (n^{-1}\bZ_A^T \bLambda_1 \bZ_A)- \frac{r}{n}\sum_{i=r+1}^p \blambda_i
    \right|
    \\
    \leq & \frac{r}{n}
    \left\|\bZ_B^T \bLambda_2 \bZ_B-(\sum_{i=r+1}^p\blambda_i)\bI_n\right\|
    =O_P\left(\frac{r}{\sqrt{n}}\|\bLambda_2\|_F+r\|\bLambda_2\|_{\infty}\right)
    .
    \end{split}
\end{equation*}
...
\begin{equation*}
    \myE \mytr (n^{-1}\bZ_B^T \bLambda_2 \bZ_B)=\mytr(\bLambda_2),
    \quad
    \myVar \left(\mytr (n^{-1}\bZ_B^T \bLambda_2 \bZ_B)\right)
...
\begin{equation*}
    \begin{split}
     ...
     \\
    =&
\mytr(\bLambda_2)+O_P\left(\frac{1}{\sqrt{n}}\|\bLambda_2\|_F\right)
    -
    \frac{r}{n}\sum_{i=r+1}^p \blambda_i
    +O_P\left(\frac{r}{\sqrt{n}}\|\bLambda_2\|_F+r\|\bLambda_2\|_{\infty}\right)
     \\
...
\begin{proposition}
    A prop for eigenvectors,
\end{proposition}
\begin{proof}
    Let $\hat{\bSigma}=n^{-1}\bY\bY^T$.

    Note that $\bU_1$ is in fact the $r$ leading eigenvectors of $\hat{\bSigma}_0=\bU_1\bU_1^T \hat{\bSigma}\bU_1\bU_1^T + n^{-1}\mytr(\bLambda_2)\bU_2 \bU_2^T$ provided the eigenvalues of $\bU_1\bU_1^T \hat{\bSigma}\bU_1\bU_1^T$ are larger than $n^{-1}\mytr(\bLambda_2)$. {\color{red}this is in fact true.}

    We apply Lemma~\ref{perturbation} with $A=\hat{\bSigma}_0$ and $\hat{A}=\hat{\bSigma}$.
    Note that
    \begin{equation*}
        \begin{split}
        ...
        +\bU_2\left(\bU_2^T \hat{\bSigma}\bU_2- n^{-1}\mytr(\bLambda_2)\bI_{p-r}\right)\bU_2^T
        \\
        =&
        n^{-1}\bU_1 \bLambda_1^{1/2} \bZ_A \bZ_B^T \bLambda_2^{1/2}\bU_2^T
        +
        n^{-1}\bU_2 \bLambda_2^{1/2} \bZ_B \bZ_A^T \bLambda_1^{1/2}\bU_1^T
        \\
        &+
        n^{-1}\bU_2\left( \bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2} -\mytr(\bLambda_2) \bI_{p-r}\right)\bU_2^T
        .
        \end{split}
    \end{equation*}
    Hence
    \begin{equation*}
        ...
        \\
        \leq &
        n^{-1}\left\|\bU_1 \bLambda_1^{1/2} \bZ_A \bZ_B^T \bLambda_2^{1/2}\bU_2^T
        +
        \bU_2 \bLambda_2^{1/2} \bZ_B \bZ_A^T \bLambda_1^{1/2}\bU_1^T
        \right\|
        \\
        &+
n^{-1}\left\| \bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}- \mytr(\bLambda_2) \bI_{p-r} \right\|
        \\
        =&
        n^{-1} \left\|\bLambda_1^{1/2} \bZ_A \bZ_B^T \bLambda_2^{1/2}\right\|
        +
        n^{-1} \left\|\bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}
-\mytr(\bLambda_2) \bI_{p-r} \right\|
        \\
        \leq&
        n^{-1} \blambda_1^{1/2}\left\| \bZ_A \bZ_B^T \bLambda_2^{1/2}\right\|
        +
        n^{-1} \left\|\bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}
-\mytr(\bLambda_2) \bI_{p-r} \right\|.
        \end{split}
    ...
    Write $\bZ_A \bZ_B^T
        = (\bZ_A \bZ_A^T)^{1/2} \bL $
    where $\bL=(\bZ_A \bZ_A^T)^{-1/2}\bZ_A\bZ_B^T$ is an $r\times (p-r)$ random matrix with independent $\mathcal{N} (0,1)$ entries.
    Then
\begin{equation*}
    \begin{split}
        %&
 \left\| \bZ_A \bZ_B^T \bLambda_2^{1/2}\right\|
        %=  \lambda_1\left((\bZ_A \bZ_A^T)^{1/2} \bL \bLambda_2 \bL^T (\bZ_A \bZ_A^T)^{1/2} \right)^{1/2}
        %\\
        \leq &
        \lambda_1\left(  \bZ_A \bZ_A^T \right)^{1/2}
        \lambda_1\left(\bL \bLambda_2 \bL^T \right)^{1/2}.
    \end{split}
\end{equation*}
By Weyl's inequality and Lemma~\ref{lemma:con}, we have
\begin{equation*}
\lambda_1\left(  \bZ_A \bZ_A^T \right)
=
n 
+
\lambda_1\left(  \bZ_A \bZ_A^T 
- n \bI_r \right)
\leq
n+
\|  \bZ_A \bZ_A^T 
- n \bI_r \|=n+O_P(\sqrt{nr}).
\end{equation*}
Similarly, we have
\begin{equation*}
    \begin{split}
...
\left\| \bZ_A \bZ_B^T \bLambda_2^{1/2}\right\|
=O_P(1)\sqrt{n \left( \mytr(\bLambda_2)
            +\sqrt{r\mytr\left(\bLambda_2^2\right)}
            +r\blambda_{r+1}
\right)}.
\end{equation}
...
If $p-r\leq n$,
the matrix $\bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}$ is full-rank. In this case,
\begin{equation*}
    \begin{split}
        &n^{-1}\left\| \bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}-
 \mytr(\bLambda_2) \bI_{p-r}\right\|
 \leq 
     n^{-1}\left\| \bZ_B^T \bLambda_2\bZ_B-\mytr(\bLambda_2)\bI_n\right\|
     .
    \end{split}
\end{equation*}
If $p-r>n$, 
the matrix $\bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}$ is of rank $n$. In this case,
\begin{equation*}
    \begin{split}
        &n^{-1}\left\| \bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}-
 \mytr(\bLambda_2) \bI_{p-r}\right\|
 \\
 =&n^{-1}\max\left(
     \left|\lambda_1\left(\bLambda_2^{1/2}\bZ_B \bZ_B^T \bLambda_2^{1/2}\right)-\mytr(\bLambda_2)\right|, 
      \mytr(\bLambda_2)
 \right)
 \\
 \leq &
    ...
    \end{split}
\end{equation*}
These bounds, togother with Lemma~\ref{lemma:con}, yield
\begin{equation*}
    \begin{split}
         n^{-1}\left\| \bLambda_2^{1/2} \bZ_B \bZ_B^T \bLambda_2^{1/2}-
 \mytr(\bLambda_2) \bI_{p-r}\right\|
...
\begin{equation*}
    ...
    =O_P(1)\left(
\sqrt{\frac{\blambda_1 \left( \mytr(\bLambda_2)
            +\sqrt{r\mytr\left(\bLambda_2^2\right)}
            +r\blambda_{r+1}
\right)}{n}}
+
...
-
            ...
            \\
            \leq &
            24\sqrt{r} \left(\frac{\|\hat{\bSigma}-\hat{\bSigma}_0\|}{\lambda_r\left(n^{-1}\bZ_A^T \bLambda_1\bZ_A  \right)-n^{-1}\mytr(\bLambda_2)}\right)^2
            =O_P(1)\sqrt{r}
...
and  for $j=1,\ldots, r$,
    \begin{equation*}
        \mathcal{R}_j=\left(n^{-1}\mytr(\bLambda_2)-\lambda_j(n^{-1}\bZ_A^T \bLambda_1 \bZ_A)\right)^{-1} \bU_2 \bU_2^T.
    \end{equation*}
    Hence
    \begin{equation*}
                f(
    ...
    where
    \begin{equation*}
        \tilde{\bLambda}_1 = \mydiag\left(\lambda_1(n^{-1}\bZ_A^T \bLambda_1 \bZ_A)-n^{-1}\mytr(\bLambda_2), \ldots, \lambda_r(n^{-1}\bZ_A^T \bLambda_1 \bZ_A)-n^{-1}\mytr(\bLambda_2)\right).
    \end{equation*}
    ...
    \begin{equation*}
        \begin{split}
            &\left\|\bLambda_1\tilde{\bLambda}_1^{-1}- \bLambda_1 \left( \bLambda_1-n^{-1} \mytr(\bLambda_2) \bI_r\right)^{-1}\right\|
            \\
= &
\max_{1\leq i\leq r}\left|
    \frac{\blambda_i}{\lambda_i(n^{-1}\bZ_A^T \bLambda_1 \bZ_A)-n^{-1}\mytr(\bLambda_2)}
    -
    \frac{1}{1-(n\blambda_i)^{-1}\mytr(\bLambda_2)}
\right|
\\
=&
\max_{1\leq i\leq r}\left|
...
It follows that
    \begin{equation*}
        \begin{split}
                ...
                -
                n^{-1} \bU_2 \bLambda_2^{1/2} \bZ_2 \bZ_1^T \bLambda_1^{1/2}\left( \bLambda_1-n^{-1} \mytr(\bLambda_2) \bI_r\right)^{-1}  \right\|
                \\
                \leq&
                \left\|
                n^{-1} \bU_2 \bLambda_2^{1/2} \bZ_2 \bZ_1^T \bLambda_1^{-1/2} 
                \left(
                \bLambda_1\tilde{\bLambda}_1^{-1}-\bLambda_1 \left( \bLambda_1-n^{-1} \mytr(\bLambda_2) \bI_r\right)^{-1}
                \right)
                \right\|
                \\
                \leq &
                ...
                \right\|
                \left\|
                \bLambda_1\tilde{\bLambda}_1^{-1}-\left(1-(n\blambda_i)^{-1}\mytr(\bLambda_2)\right)^{-1}\bI_r
                \right\|
                \\
                =&
                O_P(1)\sqrt{ \frac{r}{n^2\blambda_r}\left( \mytr(\bLambda_2)
            +\sqrt{r\mytr\left(\bLambda_2^2\right)}
            +r\blambda_{r+1}
    \right)},
        \end{split}
    ...
    \begin{equation*}
        \begin{split}
         ...
         \\
&-
n^{-1}\bU_2 \bLambda_2^{1/2} \bZ_2 \bZ_1^T \bLambda_1^{1/2}\left( \bLambda_1-n^{-1} \mytr(\bLambda_2) \bI_r\right)^{-1} \bU_1^T
\\
&-
    n^{-1}\bU_1 \left( \bLambda_1-n^{-1} \mytr(\bLambda_2) \bI_r\right)^{-1} \bLambda_1^{1/2} \bZ_1 \bZ_2^T \bLambda_2^{1/2} \bU_2^T
            \Big\|
            \\
            \leq &
            {\color{red}xxx}
...
$,
where $\bZ_1$ is a $p\times (n-k)$ matrix with i.i.d.\ $\mathcal{N}(0,1)$ entries.
We write
$
\bX\bJ\bC = \bTheta \bC + \bU\bLambda^{1/2} \bZ_2
$, 
where $\bZ_2$ is a $p\times (k-1)$ matrix with i.i.d. $\mathcal{N}(0,1)$ entries.
...
\end{equation}
where $\eta_i\overset{i.i.d.}{\sim} \mathcal{N}(0,\bI_{k-1})$, $i=1,\ldots,p$.
...
%$
%\bY^T \bY=
%\bZ_1^T \bLambda \bZ_1   = \bV_{\bY} \bD_{\bY}^2 \bV_{\bY}^T
%$, and 
%$
%\bZ_1^T \bLambda \bZ_1=
...
\end{aligned}
$$
%where $\bW_{k-1}$ is a $(k-1)\times(k-1)$ symmetric random matrix whose entries above the main diagonal are i.i.d.\ $\mathcal{N}(0,1)$ and the entries on the diagonal are i.i.d.\ $\mathcal{N}(0,2)$.
