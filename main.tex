...
%The likelihood ratio test (LRT) has a dominated position in classical multivariate analysis.
   Note that both sum-of-squares type statistic and extreme value type statistic are not based on likelihood function.
    ...
    {\color{red} unbounded likelihood
Inspired by Roy's union-intersection principle \citep{Roy1953},
    ...
    They use a {least favorable} argument to construct a test statistic based on component tests.
}
    ...
    Our theoretical results imply that the asymptotic power of the new test is not affected by large eigenvalues, while most existing tests are negatively affected by large eigenvalues.
