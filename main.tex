...
\end{pmatrix}.
 $$
The matrix $\tilde{\bJ}$ is a column orthogonal matrix  satisfying $\tilde{\bJ}^\top\tilde{\bJ}=\bI_{n-k}$ and $\tilde{\bJ}\tilde{\bJ}^\top =\bI_n-\bJ\bJ^\top$.
...
$$
By some matrix algebra, we have $\bI_k-\frac{1}{n}\bJ^\top\mathbf{1}_n \mathbf{1}_n^\top \bJ=\bC\bC^\top$
...
%Then the matrices $I_n-JJ^\top$, $JJ^\top-\frac{1}{n}\mathbf{1}_n\mathbf{1}_n^\top$ and $\frac{1}{n}\mathbf{1}_n\mathbf{1}_n^\top$ are three $n\times n$ projection matrices which are pairwise orthogonal with rank $n-k$, $k-1$ and $1$.
%Let $\tilde{J}$ be a $n\times (n-k)$ matrix satisfying $\tilde{J}\tilde{J}^\top =I-JJ^\top$.
%Note that $I_k-\frac{1}{n}J^\top\mathbf{1}_n \mathbf{1}_n^\top J$ is a $k\times k$ projection matrix with rank $k-1$.
%Let $C$ be a $k\times (k-1)$ matrix satisfying $CC^\top=I_k-\frac{1}{n}J^\top\mathbf{1}_n \mathbf{1}_n^\top J$.
%Then 
%$$
%G=Z(I_n-JJ^\top)Z^\top=
%Z\tilde{J}\tilde{J}^\top Z^\top.
%$$
%and
%\begin{equation*}
    %\begin{aligned}
        %F=\sum_{i=1}^k n_i (\bar{\mathbf{X}}_i-\bar{\mathbf{X}})(\bar{\mathbf{X}}_i-\bar{\mathbf{X}})^\top 
        %=Z(JJ^\top-\frac{1}{n}\mathbf{1}_n\mathbf{1}_n^\top)Z^\top
%=ZJ(I_k-\frac{1}{n}J^\top\mathbf{1}_n \mathbf{1}_n^\top J)J^\top Z^\top
%=ZJC C^\top J^\top Z^\top.
    %\end{aligned}
%\end{equation*}
Let $\bY=\bU_{\bY}\bD_{\bY}\bV_{\bY}^\top$ be the singular value decomposition of $\bY$, where $\bU_{\bY}$ and $\bV_{\bY}$ are $p\times (n-k)$ and $(n-k)\times(n-k)$ column orthogonal matrices, $\bD_{Y}$ is an $(n-k)\times (n-k)$ diagonal matrix.
...
\begin{equation}\label{statisticForm2}
    \begin{aligned}
        T(\bX)=
        \lambda_{\max}\Big(\bC^\top\big( \bJ^\top (\bX^\top \bX)^{-1}\bJ \big)^{-1}\bC\Big).
    \end{aligned}
\end{equation}
...
%Note that $CM\sqrt{n_i}\bar{\bX}_i\sim \mathcal{N}_{k-1}(\sqrt{n_i}CM\xi_i,CM\bSigma M^\top C^\top)$.
%Hence we have that
%$$
%\mytr \big(CMZJ\big)
%\sim
...
In the sparse case, we set $\xi_1=\kappa (1_{p/5}^\top,\mathbf{0}_{4p/5}^\top)^\top$, $\xi_2=\kappa (\mathbf{0}_{p/5}^\top, 1_{p/5}^\top,\mathbf{0}_{3p/5}^\top)^\top$ and $\xi_3=\mathbf{0}_p$.
...
We set $\xi_1=\kappa (\textbf{1}_{p/2}^\top,-\textbf{1}_{p/2}^\top)^\top$ and $\xi_2=\mathbf{0}_p$, where $\kappa$ is selected such that SNR equals to $5$.
...
\begin{lemma}[von Neumann's trace theorem]
    Let $\bA$ and $\bB$ be two $m\times n$ matrices. Let $\sigma_1(\bA)\geq \ldots \geq \sigma_q (\bA)$ and $\sigma_1(\bB)\geq \ldots \geq \sigma_q (\bB)$ denote the non-increasingly ordered singular values of $\bA$ and $\bB$, respectively. Then
    \begin{equation*}
...
\end{proposition}
\begin{proof}
    The first statement is a direct consequence of Proposition~\ref{eigenvalueProp} and the condition $r=o(n)$.
...
\bP_{\bY,1}^{\dagger}
    ...
    \begin{equation}\label{UpperBoundQ}
        ...
        \|\bQ^\top \bQ \|
        =& \left\|
        \bLambda_1^{-1/2} (\bZ_1 \bZ_1^T)^{-1/2} \bV_{\bZ_1}^T \bZ_2^T \bLambda_2 \bZ_2 \bV_{\bZ_1} (\bZ_1 \bZ_1^T)^{-1/2} \bLambda_1^{-1/2}
        \right\|
        \\
        \leq &
...
\begin{equation*}
    \begin{split}
         ...
         +
         \left\|
         \bLambda_1^{1/2} (\bZ_1 \bZ_1^\top)^{-1} \bZ_1 \bZ_2^\top \bLambda_2^{1/2}
         \right\|
         +
...
\begin{equation*}
    \begin{split}
            ...
            \\
            =&
            (\bV_{\bZ_1}^\top \bZ_2^\top \bLambda_2^2 \bZ_2 \bV_{\bZ_1})^{1/2}
            \bZ_2^{\dagger}
            \bZ_2^{\dagger T}
            (\bV_{\bZ_1}^\top \bZ_2^\top \bLambda_2^2 \bZ_2 \bV_{\bZ_1})^{1/2}.
    \end{split}
...
\begin{equation*}
    \begin{split}
            ...
            \right\|
            \left\|\bV_{\bZ_1}^\top \bZ_2^\top \bLambda_2^2 \bZ_2 \bV_{\bZ_1}\right\|
            \\
            \leq&
            \blambda_{r+1}
            \left\|
...
\begin{proof}[Proof of Proposition~\ref{traceProp1}]
    ...
    $$
    \frac{1 }{n-k}
        \sum_{i=r+1}^{n-k}\lambda_i(\bY^\top\bY)
        =\mytr(\bLambda_2)+O_P(\sqrt{p}).
    $$
    Note that
$$
\bY^\top \bY =\bZ_1^\top \bLambda \bZ_1
=
\bZ_{1A}^\top \bLambda_1 \bZ_{1A}+
\bZ_{1B}^\top \bLambda_2 \bZ_{1B}.
$$
By Weyl's inequality, for $i=r+1,\ldots, n-k$, we have
$$
\lambda_{i}(\bZ_{1B}^\top \bLambda_2 \bZ_{1B})\leq \lambda_i(\bY^\top \bY)\leq \lambda_{i-r}(\bZ_{1B}^\top \bLambda_2 \bZ_{1B}).
$$
It follows that
$$
\sum_{i=r+1}^{n-k}\lambda_{i}(\bZ_{1B}^\top \bLambda_2 \bZ_{1B})\leq \sum_{i=r+1}^{n-k}\lambda_i(\bY^\top \bY)\leq \sum_{i=1}^{n-k-r}\lambda_{i}(\bZ_{1B}^\top \bLambda_2 \bZ_{1B}).
$$
Hence
$$
 \big|\sum_{i=r+1}^{n-k}\lambda_i(\bY^\top \bY)- \mytr(\bZ_{1B}^\top \bLambda_2 \bZ_{1B})\big|\leq r \lambda_1(\bZ_{1B}^\top \bLambda_2 \bZ_{1B})=O_P(rp),
$$
    Where the last equality holds by Lemma~\ref{DSbound}.
But central limit theorem implies that
$$\mytr(\bZ_{1B}^\top \bLambda_2 \bZ_{1B})-(n-k)\mytr(\bLambda_2)%=\sqrt{(n-k)\mytr(\bLambda_2^2)}O_P(1)
=O_P(\sqrt{np}).$$
Thus,
$$
    \begin{aligned}
        &\frac{1}{n-k}\sum_{i=r+1}^{n-k}\lambda_i (\bY^\top \bY)\\
        =&\mytr(\bLambda_2)+
        \frac{1}{n-k}\big(\sum_{i=r+1}^{n-k}\lambda_i(\bY^\top \bY)-\mytr(\bZ_{1B}^\top \bLambda_2 \bZ_{1B})\big) +\frac{1}{n-k}\big(\mytr(\bZ_{1B}^\top \bLambda_2 \bZ_{1B})-(n-k)\mytr(\bLambda_2)\big)\\
...
\end{proof}
    ...
    \begin{proof}[Proof of Proposition~\ref{Taichangle}]
        ...
        Let $\bU_{\bY,2;(i,j)}$ be a $p\times (p-r)$ orthogonal matrix satisfying $\bU_{\bY,2;(i,j)}\bU_{\bY,2;(i,j)}^\top= \bI_p-\bU_{\bY,1;(i,j)}\bU_{\bY,1;(i,j)}^\top$.
        Then by Lemma~\ref{PCAlemma2}, we have
        $$
        \myE\lambda_1(\bU_1^\top \bU_{\bY,2;(i,j)}\bU_{\bY,2;(i,j)}^\top \bU_1)
        =
        \myE\lambda_1(\bI_r-\bU_1^\top \bU_{\bY,1;(i,j)}\bU_{\bY,1;(i,j)}^\top \bU_1)
        =
        O(\frac{p}{\blambda_r n})
        $$
        and
        $$
        \myE\lambda_1^2(\bU_1^\top \bU_{\bY,2;(i,j)}\bU_{\bY,2;(i,j)}^\top \bU_1)
        =
        O(\frac{p^2}{\blambda_r^2 n^2}).
        $$


        First, we prove that $w_{ij}^2$ is an approximation of $Y_i^\top \bU_2 \bU_2^\top Y_j$.
        For $1\leq i<j\leq n-k$, define $\epsilon_{ij}= w_{ij}-Y_i^\top \bU_{2} \bU_{2}^\top Y_j$, then we have
        \begin{equation}\label{emLong}
    \begin{aligned}
        &\epsilon_{ij}
        =Y_i^\top (\tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top-\bU_2 \bU_2^\top) Y_j\\
        =&Y_i^\top(\bU_1 \bU_1^\top+\bU_2 \bU_2^\top) (\tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top-\bU_2 \bU_2^\top)(\bU_1 \bU_1^\top+\bU_2 \bU_2^\top) Y_j\\
        =&Y_i^\top \bU_2 \bU_2^\top (\tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top-\bU_2\bU_2^\top) \bU_2 \bU_2^\top Y_j\\
        &+Y_i^\top \bU_1 \bU_1^\top \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top \bU_2 \bU_2^\top Y_j\\
        &+Y_i^\top \bU_2 \bU_2^\top \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top \bU_1 \bU_1^\top Y_j\\
        &+Y_i^\top \bU_1 \bU_1^\top \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top \bU_1 \bU_1^\top Y_j\\
        =&
            Y_i^\top\bU_2 \bU_2^\top ( \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top-\bU_{\bY,2;(i,j)} \bU_{\bY,2;(i,j)}^\top)\bU_2 \bU_2^\top Y_j\\
            &+
            Y_i^\top \bU_2 \bU_2^\top(\bU_{\bY,2;(i,j)} \bU_{\bY,2;(i,j)}^\top-\bU_2 \bU_2^\top)\bU_2 \bU_2^\top Y_j\\
        &+Y_i^\top \bU_1 \bU_1^\top \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top \bU_2 \bU_2^\top Y_j\\
        &+Y_i^\top \bU_2 \bU_2^\top \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top \bU_1 \bU_1^\top Y_j\\
        &+Y_i^\top \bU_1 \bU_1^\top \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top \bU_1 \bU_1^\top Y_j\\
        \overset{def}{=} &
    ...
    \end{equation}
        ...
        Note that
        $$
\bU_{\bY,2;(i,j)} \bU_{\bY,2;(i,j)}^\top
-
        \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top
        =
\bU_{\bY;(i,j)} \bU_{\bY;(i,j)}^\top-
        \bU_{\bY,1;(i,j)} \bU_{\bY,1;(i,j)}^\top
        $$
        ...
        \begin{aligned}
            &\myE (\epsilon_{ij}^{(1)})^2\\
            %&\myE\|Y_i^\top\bU_2 \bU_2^\top ( \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top-\bU_{\bY,2;(i,j)} \bU_{\bY,2;(i,j)}^\top)\bU_2 \bU_2^\top Y_j\|^2\\
            =&\myE \mytr (\bLambda_2^{1/2}\bU_2^\top(\tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top-\bU_{\bY,2;(i,j)} \bU_{\bY,2;(i,j)}^\top)\bU_2 \bLambda_2^{1/2})^2\\
            \leq & c_1^2\myE \mytr (\tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top-\bU_{\bY,2;(i,j)} \bU_{\bY,2;(i,j)}^\top)^2\\
        ...
        \begin{aligned}
            &\myE (\epsilon_{ij}^{(2)})^2
            =\myE \mytr (\bLambda_2^{1/2}\bU_2^\top(
            \bU_{\bY,2;(i,j)} \bU_{\bY,2;(i,j)}^\top-\bU_2\bU_2^\top
...
\bU_{\bY,1;(i,j)} \bU_{\bY,1;(i,j)}^\top
            ...
            = & c_1^2 
            \myE \mytr (\bU_2^\top\bU_{\bY,1;(i,j)} \bU_{\bY,1;(i,j)}^\top\bU_2 )^2\\
            \leq  & c_1^2 
            \myE \mytr (\bU_{\bY,1;(i,j)} \bU_{\bY,1;(i,j)}^\top)^2
            =c_1^2 r=o(p).
        \end{aligned}
        $$
        ...
        \begin{aligned}
            &\myE(\epsilon_{ij}^{(3)})^2 =
            \myE(\epsilon_{ij}^{(4)})^2 \\
            =&\myE \mytr ( \bLambda_1^{1/2}\bU_1^\top\tilde{\bU}_{\bY;(i,j)}\tilde{\bU}_{\bY;(i,j)}^\top \bU_2 \bLambda_2 \bU_2^\top \tilde{\bU}_{\bY;(i,j)}\tilde{\bU}_{\bY;(i,j)}^\top \bU_1 \bLambda_1^{1/2})\\
            \leq& c_1 \blambda_1  \myE \mytr ( \bU_1^\top\tilde{\bU}_{\bY;(i,j)}\tilde{\bU}_{\bY;(i,j)}^\top \bU_2 \bU_2^\top \tilde{\bU}_{\bY;(i,j)}\tilde{\bU}_{\bY;(i,j)}^\top \bU_1 )\\
            \leq& c_1 \blambda_1  \myE \mytr ( \bU_1^\top\tilde{\bU}_{\bY;(i,j)}\tilde{\bU}_{\bY;(i,j)}^\top \bU_1 )\\
            \leq& c_1 \blambda_1  \myE \mytr ( \bU_1^\top{\bU}_{\bY,2;(i,j)}{\bU}_{\bY,2;(i,j)}^\top \bU_1 )\\
        ...
        \begin{aligned}
            \myE(\epsilon_{ij}^{(5)})^2 
            =&\myE \mytr (\bLambda_1^{1/2}\bU_1^\top
            \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top\bU_1 \bLambda_1^{1/2})^2\\
            \leq & \blambda_1^2\myE \mytr (\bU_1^\top
            \tilde{\bU}_{\bY;(i,j)} \tilde{\bU}_{\bY;(i,j)}^\top\bU_1 )^2\\
            \leq&  \blambda_1^2  \myE \mytr ( \bU_1^\top{\bU}_{\bY,2;(i,j)}{\bU}_{\bY,2;(i,j)}^\top \bU_1 )^2\\
...
Hence we have bound $\bE (\epsilon_{ij}^2)=o(p)$.
    ...
    $$
        \begin{aligned}
            &\widehat{\mytr(\bLambda_2^2)}
            =
        \frac{2}{(n-k)(n-k-1)}\sum_{1\leq i<j\leq n-k}(Y_i^\top \bU_2 \bU_2^\top Y_j)^2 \\
            &+
            \frac{2}{(n-k)(n-k-1)}\sum_{1\leq i<j\leq n-k}\big(2\epsilon_{ij}(Y_i^\top \bU_2 \bU_2^\top Y_j)
            + \epsilon_{ij}^2\big).
        \end{aligned}
    $$
        We have
        $$
            \begin{aligned}
                &\myE\Big|\frac{2}{(n-k)(n-k-1)}\sum_{1\leq i<j\leq n-k}\big(2\epsilon_{ij}(Y_i^\top \bU_2 \bU_2^\top Y_j)
                +
                \epsilon_{ij}^2\big)\Big|\\
                \leq&
                \myE\Big|2\epsilon_{12}(Y_1^\top \bU_2 \bU_2^\top Y_2)
            +
            \epsilon_{12}^2\Big|\\
            \leq&
                2\sqrt{\myE (\epsilon_{12}^2)\myE\big(Y_1^\top \bU_2 \bU_2^\top Y_2\big)^2}
            +
...
$$
    ...
    \frac{2}{(n-k)(n-k-1)}\sum_{1\leq i<j\leq n-k}(Y_i^\top \bU_2 \bU_2^\top Y_j)^2 
        $$
        is ratio consistent.
        Since $\myE (Y_i^\top \bU_2 \bU_2^\top Y_j)^2=\mytr(\bLambda_2^2)$ for $i< j$, we have
    $$
        \myE
    \frac{2}{(n-k)(n-k-1)}\sum_{1\leq i<j\leq n-k}(Y_i^\top \bU_2 \bU_2^\top Y_j)^2 
        =\mytr(\bLambda_2^2).
    ...
    \frac{2}{(n-k)(n-k-1)}\sum_{1\leq i<j\leq n-k}(Y_i^\top \bU_2 \bU_2^\top Y_j)^2 
            \Big)^2\\
            =&
            \frac{4}{(n-k)^2(n-k-1)^2}\Big(\sum_{1\leq i < j\leq n-k}(Y_i^\top \bU_2 \bU_2^\top Y_j)^2\Big)^2\\
            =&
            \frac{4}{(n-k)^2(n-k-1)^2}\myE\Big( \sum_{i<j}(Y_i^\top\bU_2 \bU_2^\top Y_j)^4\\
            &+
            \sum_{i<j,k<l:\{i,j\}\cap\{k,l\}=\emptyset}(Y_i^\top\bU_2 \bU_2^\top Y_j)^2 (Y_k^\top\bU_2 \bU_2^\top Y_l)^2\\
            &+2\sum_{i<j<k}\big((Y_i^\top\bU_2 \bU_2^\top Y_j)^2(Y_i\bU_2 \bU_2^\top Y_k)^2\\
            &+(Y_i^\top\bU_2 \bU_2^\top Y_j)^2(Y_j\bU_2 \bU_2^\top Y_k)^2+(Y_i^\top\bU_2 \bU_2^\top Y_k)^2(Y_j\bU_2 \bU_2^\top Y_k)^2\big) \Big)\\
            =&
    \frac{4}{(n-k)^2(n-k-1)^2}\Big(
        ...
        $$
        \myVar
    \Big(\frac{2}{(n-k)(n-k-1)}\sum_{1\leq i<j\leq n-k}(Y_i^\top \bU_2 \bU_2^\top Y_j)^2 
    \Big)
            =
            o(\mytr^2(\bLambda_2^2)).
        $$
