...
\end{proof}
\begin{proposition}
    Suppose that $r=o(n)$ and $\mytr(\bLambda_2)\blambda_1/(n\blambda_r^2)\to 0$.
    Then
    \begin{equation*}
        \sum_{i=r+1}^p \lambda_i (\hat{\bSigma}^2)=
    \end{equation*}
    \label{eigenvalueProp:R3}
\end{proposition}
\begin{proof}
    We only need to deal with $n^{-2}(\bZ^T \bLambda \bZ)^2$ since it shares the same non-zero eigenvalues as $\hat{\bSigma}^2$.
    It can be seen that
    \begin{equation*}
        n^{-2}(\bZ^T \bLambda \bZ)^2
        =
        \bR+ n^{-2} (\bZ_2^T \bLambda_2 \bZ_2)^2,
    \end{equation*}
    where 
    $
    \bR=n^{-2}( \bZ_1^T \bLambda_1 \bZ_1)^2+n^{-2} \bZ_1^T \bLambda_1 \bZ_1\bZ_2^T \bLambda_2 \bZ_2+n^{-2} \bZ_2^T \bLambda_2 \bZ_2\bZ_1^T \bLambda_1 \bZ_1
    $
    is a symmetric matrix with rank at most $2r$.
    Furthermore, the condition $r=o(n)$ and $\mytr(\bLambda_2)\blambda_1/(n\blambda_r^2)\to 0$ imply that $\bR$ is positive definite with probability tending to $1$.
    To see this, note that~\eqref{eigenBoundForA} and~\eqref{conB2B1} imply that $\lambda_r(n^{-2}( \bZ_1^T \bLambda_1 \bZ_1)^2)=(1+o_P(1))\blambda_r^2$, and~\eqref{} implies that
