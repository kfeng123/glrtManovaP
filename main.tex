...
\begin{theorem}
    ...
    Then
    \begin{equation*}
        \frac{T(\bX)-\left(\mytr(\bSigma)-n\mytr(\bSigma^2)/\mytr(\bSigma)\right)}{\sqrt{\mytr(\bSigma^2)-n\mytr^2(\bSigma^2)/\mytr^2(\bSigma)}}
        \sim
        \lambda_1\Big(\bW_{k-1}+\mytr^{-1/2}(\bSigma^2) \bC^\top \bTheta^\top (\bI_p-\bP_{\bY})\bTheta \bC\Big)
        +o_P(1),
    \end{equation*}
    where $\sim$ means having the same distribution.
\end{theorem}
\begin{remark}
    The condition $n\blambda_{1}/\mytr(\bLambda_2)\to 0$ implies that $p/n \to \infty$. Hence $T(\bX)$ is well defined for large $n$.
\end{remark}
...
\end{proposition}
\begin{remark}
    The condition $n\blambda_{r+1}/\mytr(\bLambda_2)\to 0$ implies that $p/n \to \infty$.
    Hence $\bP_{\bY,2}$ is well defined for large $n$.
