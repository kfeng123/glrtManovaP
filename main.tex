...
In low dimensional setting, the problem is well studied.
The difficulty occurs when $p\geq n$ where the four classical test statistics can not be defined.
As the high dimensional problem is hard to deal with, an simple idea is to reduce the problem into a class of univariate problems.
...
step 1 and step 3 are more flexible.
As for step 3, Roy's union intersection principle suggest to use $\max_{\gamma\in\Gamma}T_{\gamma}$ as global test statistic~\citep{Roy1953}.
Another choice is to integrate $T_\gamma$ according some measure $\mu(\gamma)$ and use $\int_\gamma T_{\gamma}\,\mu(d\gamma)$ as global test statistic.
...
By using data $\Omega^{-1}\bX$ and component test statistics
$$
...
Next, we consider using construction ii in step 1, which is not relies on the basis of $\mathbb{R}^p$.
...
More specifically, one can randomly generate unit vectors $a_1,\ldots,a_M$ and the statistics can be approximated by $M^{-1}\sum_{i=1}^M a^T \bH a/a^T \bG a$.
This statistic is well defined in high dimensional setting.
A similar method is proposed by~\cite{Lopes2015A} for $k=2$ from different point of view.
...
Hence we define $a^* = \argmin_{a\in\mathcal{A}} L_0 (a)$ and take $H_{a^*}$ as the least favorable hypothesis. Equivalently
$$
\begin{aligned}
...
While~\eqref{statisticForm1} is convenient for theoretical analysis,~\eqref{statisticForm2} is suitable for computation.

\section{Main results}
We shall derive the asymptotic distribution of $T(\bX)$.
...
\begin{assumption}\label{assumpEigen}
 ...
 $c_1 \geq \blambda_{r+1} \geq \ldots \geq \blambda_{p} \geq c_2$ for some absolute constants $c_1$ and $c_2$.
    ...
    \frac{\blambda_1^2 pr^2}{\blambda_r^2 n^2}\to 0.$$
\end{assumption}
...
We use the following statistic to estimate $r$:
    $$\hat{r}=\argmax_{0\leq i\leq n-k} \frac{\lambda_i(\bY^T \bY)}{\lambda_{i+1}(\bY^T \bY)}\geq \gamma_n,$$
    where $\gamma_n$ slowly tends to $+\infty$ as $n\to \infty$.
...
\begin{proposition}
    Under Assumption~\ref{assumpEigen}, suppose $p/n\to \infty$, $\gamma_n\to \infty$ and $\gamma_n=o({n\blambda_r}/{p})$, then
    $$\widehat{\mytr(\bLambda_2)}=\mytr(\Lambda_2)+o_P(\sqrt{p}).$$
\end{proposition}
\begin{proof}
    Lemma~\ref{PCAlemma1} and Assumption~\ref{assumpEigen} imply that
    $$
    \frac{1}{n-k}\sum_{i=r+1}^{n-k}\lambda_i (\bY^T \bY)= \mytr(\bLambda_2)+O_P\big(\frac{rp}{n}\big)=\mytr(\bLambda_2)+o_P(\sqrt{p}).
    $$
    It only remains to prove $\hat{r}$ is a consistent estimator of $r$. $\Pr(\hat{r}=r)\to 1$.
    Note that 
    $$
    \{\hat{r}=r\}\supseteq 
    ...
    \frac{\lambda_{r+1}(\bY^T \bY)}{\lambda_{n-k}(\bY^T \bY)}\leq (1+\epsilon)\frac{c_1}{c_2}.
    $$
    This yields $\Pr(\hat{r}=r)\to 1$, which completes the proof.
\end{proof}
...
It follows that $w_{ij}^2=(Y_i^T \bU_2 \bU_2^T Y_j)^2+o_P(p)$. Thus
$$
...
\begin{lemma}\label{PCAlemma1}
    Under the Assumptions of Theorem~\ref{thm1}, $r=o(n)$, we have
    \begin{align}
    ...
    \end{align}
    almost surely. And
    \begin{equation}\label{PCAlemma1:eq4}
    \frac{
       1 
    }{
        n-k
...
Now we prove~\eqref{PCAlemma1:eq4}. By Weyl's inequality, for $i=r+1,\ldots, n-k$, we have
$$
...
Thus
$$
\big|\sum_{i=r+1}^{n-k}\lambda_i(\bY^T \bY)- (n-k)\mytr(\bLambda_2)\big|=O_P(rp).
$$
This completes the proof.
