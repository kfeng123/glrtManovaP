...
\begin{proposition}\label{eigenvectorProp}
    Suppose that $p>n$, $r=o(n)$, ${\blambda_1 \mytr(\bLambda_2)}/{(n \blambda_r^2)}\to 0$ and $n\blambda_{r+1} /\mytr(\bLambda_2)\to 0$. Then
    \begin{equation*}
        \begin{split}
            &\Big\|
         \bU_{\bY,1}\bU_{\bY,1}^T
...
+\blambda_{r+1}\right).
\end{equation*}
This, combined with~\eqref{firstTermNao}, leads to
\begin{equation*}
    \begin{split}
    &\|\hat{\bSigma}-\hat{\bSigma}_0\|
    =O_P\left(
...
\begin{proposition}
    Suppose that $p>n$, $r=o(n)$, ${\blambda_1 \mytr(\bLambda_2)}/{(n \blambda_r^2)}\to 0$, $n\blambda_{r+1} /\mytr(\bLambda_2)\to 0$ and $\blambda_{r+1}/\blambda_r\to 0$. Then
...
\begin{proof}
    ...
    Proposition~\ref{eigenvectorProp} implies that $\bU_{\bY,1}\bU_{\bY,1}^T$ can be approximated by
     $\bP^*_{\bY,1}=
         \bU_1\bU_1^T
+
n^{-1}\bU_2 \bLambda_2^{1/2} \bZ_2 \bZ_1^T \bLambda_1^{-1/2} \bU_1^T
+
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    In the current context, Lemma~\ref{eigenvalueProp} implies that
    \begin{equation*}
    ...
    \begin{equation*}
        \begin{split}
             ...
             \right)
                 \left(
                     \left(\frac{\sqrt{r}\blambda_1 \mytr(\bLambda_2)}{n\blambda_r^2}\right)^{2}
                     +\left(\frac{\sqrt{r}\blambda_{r+1}^2}{\blambda_r^2}\right)^{2}
                     +\left(\frac{r}{n}\right)^{2}
                 \right)
             .
        \end{split}
    \end{equation*}
...
\right)
    ...
    \begin{equation*}
        \begin{split}
             ...
             \right)
                 \left(
                     \left(\frac{\sqrt{r}\blambda_1 \mytr(\bLambda_2)}{n\blambda_r^2}\right)^{3/2}
                     +\left(\frac{\sqrt{r}\blambda_{r+1}^2}{\blambda_r^2}\right)^{3/2}
                     +\left(\frac{r}{n}\right)^{3/2}
                 \right).
        \end{split}
    \end{equation*}
...
\leq&
             n^{-2} 
             \blambda_r^{-1/2}
            \|\bI_n -n^{-1} \bZ_1^T \bZ_1\|
            \|\bZ_2^T \bLambda_2\bZ_2\|
            \left\| \bLambda_2^{1/2}  \bZ_2 \bZ_1^T \right\|
             +
             n^{-3}
             \blambda_r^{-1}
            \left\| \bLambda_2^{1/2}  \bZ_2 \bZ_1^T \right\|^2
            \left\|\bZ_2^T \bLambda_2 \bZ_2\right\|
\\
=&O_P(1)\sqrt{\frac{\mytr(\bLambda_2)}{n\blambda_r}}\left(\blambda_{r+1}+\frac{\mytr(\bLambda_2)}{n}\right)
+O_P(1)\frac{\mytr(\bLambda_2)}{n\blambda_r}\left(\blambda_{r+1}+\frac{\mytr(\bLambda_2)}{n}\right)
\\
=&O_P(1)\sqrt{\frac{\mytr(\bLambda_2)}{n\blambda_r}}\left(\blambda_{r+1}+\frac{\mytr(\bLambda_2)}{n}\right)
,
        \end{split}
    \end{equation*}
    where the second last equality follows from~\eqref{firstTermNao} and~\eqref{ineqForZ2}.
