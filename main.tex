...
%$$
%f_a(\bX_a;\xi_1,\ldots,\xi_k,\bSigma)=
    %\frac{1}{(2\pi)^{n/2}|a^T \bSigma a|^{n/2}}
    %\exp\Big(-\frac{1}{2 a^T \bSigma a}\sum_{i=1}^k\sum_{j=1}^{n_i}(a^T X_{ij}-a^T\xi_i)^2\Big).
%$$
%Then the likelihood ratio test statistic for $H_a$ against $K_a$ is 
%Note that
%\begin{equation*}
    %\begin{aligned}
        %\max_{\xi_1,\ldots,\xi_k,\bSigma}f_a(a^T Z,\xi_1,\ldots,\xi_k,\bSigma)
        %=
...
%\begin{equation*}
    %\begin{aligned}
        %\max_{\xi,\bSigma}f_a(a^T Z,\xi,\ldots,\xi,\bSigma)
        %=
...
\begin{equation*}
    \begin{aligned}
        \text{LR}_{a}%=\frac{\sup\limits_{\xi_1,\ldots,\xi_k,\bSigma}f_a(a^T \bX;\xi_1,\ldots,\xi_k,\bSigma)}{\sup\limits_{\xi,\bSigma}f_a(a^T \bX;\xi,\ldots,\xi,\bSigma)}
        =
...
%%$$
%f(Z;\xi_1,\ldots,\xi_k,\bSigma)=\prod_{i=1}^k\Big[
    %(2\pi)^{-n_i p/2}|\bSigma|^{-n_i/2}\exp(-\frac{1}{2}\mathrm{tr}\bSigma^{-1}\sum_{j=1}^{n_i}(x_{ij}-\xi_i)(x_{ij}-\xi_i)^T)
    %\Big].
%$$
...
\begin{theorem}\label{nonSpiked}
    ...
    we have
    %$$
    %{\Big(2\mytr (\bSigma^2)\Big)}^{-1/2}{\Big(C^TJ^T Z^T(\bI_p-\bP_{Z\tilde J}) ZJC- \frac{p-n+k}{p}\mytr(\bSigma)\bI_{k-1}-C^T \Xi^T (\bI_p-\bP_{Z\tilde{J}})\Xi C}\Big)
