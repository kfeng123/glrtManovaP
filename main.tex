...
\begin{theorem}\label{thm1}
    ...
    \end{aligned}
    $$
    where $\sim$ means they have same distribution.
\end{theorem}
...
While for $r>0$, ${2\mytr (\bLambda_2^2)}$ can be smaller than $2\mytr(\bSigma^2)$.
...
$$
The reason for this is because the projection $\bI_p-\bP_{\bY}$ appeared in expression~\eqref{statisticForm1} can remove large variance term of $\bX\bJ \bC$.
...
$$
Let $F_{\bW}(x)$ be the cumulative distribution function of $\bW_{k-1}$.
We reject the null hypothesis if $Q> F_{\bW}^{-1}(1-\alpha)$.
...
And the asymptotic local power function can be given by Theorem~\ref{thm1}, which is
$$
\Pr\Big(
...
$$
the new test tends to be more powerful.
...
\subsection{Permutation method}
 ...
 To compare the new test with other test, we use permutation method to determine the critical value in our simulation.
...
Our theoretic work shows that when the covariance matrix is spiked, the asymptotic power of the proposed test is not affected by the large eigenvalues and is more powerful than existing tests when variables are highly correlated.
...
\begin{proof}[Proof of Proposition~\ref{numberConsistency}]
    ...
    The case of $r=0$ can be similarly proved by note that in this case,
    $$
    \{\hat{r}=r\}\supseteq 
